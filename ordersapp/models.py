...
class Order(models.Model):
    ...
    def get_total_weeks(self):
        ...
        return sum(list(map(lambda x: x.weeks, accommodations)))


    def get_total_cost(self):
        accommodation = self.orderitems.select_related()
        return sum(list(map(lambda x:
		x.weeks * x.accommodation.price, accommodation)))

    def delete(self):
        for item in self.orderitems.select_related():
            item.accommodation.availability += item.weeks
...
class OrderItem(models.Model):
    ...
    weeks = models.PositiveIntegerField(verbose_name='количество', default=0)

    def get_total_cost(self):
        accommodation = self.orderitems.select_related()
        return sum(list(map(lambda x: x.weeks * x.accommodation.price, accommodation)))
